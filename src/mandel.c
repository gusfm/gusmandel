...
void mandel_calculate(mandel_t *mandel, palette_t *pal)
{
    ...
    for (x = rmin, j = 0; j < width; x += delta, j++) {
        for (y = imin, i = 0; i < height; y -= delta, i++) {
            mandel->img->rgb_matrix[j][i]
                = palette_get_color(pal, mandel_iter(x, y, pal->max_iter));
        }
    }
}
...
void mandel_calculate_smooth(mandel_t *mandel, palette_t *pal)
{
    ...
    for (x = rmin, j = 0; j < width; x += delta, j++) {
        for (y = imin, i = 0; i < height; y -= delta, i++) {
            palette_get_color_smooth(pal, &mandel->img->rgb_matrix[j][i],
                                     mandel_iter_smooth(x, y, pal->max_iter));
        }
    }
}
...
void mandel_calc_range(mandel_t *mandel, double delta, int j, int width)
{
    ...
    while (j < width) {
        for (y = mandel->imin, i = 0; i < mandel->height; y -= delta, i++) {
            palette_get_color_smooth(
                mandel->pal, &mandel->img->rgb_matrix[j][i],
                mandel_iter_smooth(x, y, mandel->pal->max_iter));
        }
        x += delta;
        j++;
    }
...
void *mandel_thread_run(void *arg)
{
    ...
    while ((work_id = mandel_get_work()) <= max_jobs) {
        ...
        x_work_end = work_width_tmp < mandel->width ? work_width_tmp
                                                    : mandel->width;
...
mandel_t *mandel_create(image_t *image, palette_t *pal, double real,
                        double imag, double radius)
{
    mandel_t *mandel;
...
void mandel_destroy(mandel_t *mandel)
{
    free(mandel);
}
